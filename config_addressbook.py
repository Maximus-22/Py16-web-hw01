...
class Field(ABC):
    ...
    @abstractmethod
    def validate_entity(value):
        pass


class Address(Field):
    def __init__(self, value):
        if not self.validate_entity(value):
            raise ValueError("Invalid address")
        super().__init__(value)

    def validate_entity(value):
        address_pattern = re.compile(r'^[A-ZА-Я]{1}[\w,\s]+')
...
class Name(Field):
    ...
    def validate_entity(value):
        name_pattern = re.compile(r'^[A-ZА-Я]{1}[\s\D]+')
...
class Record:
    def __init__(self, name: Name, phones: list, address: Address = None, emails: list = None, birthday: Birthday = None):
        self.name = str(name)
    ...
    def process_phones(self, phones):
        ...
        for phone_field in phones:
            ...
            for phone_number in phone_numbers:
                if Phone.validate_entity(phone_number):
